...
const CDN_URL = 'https://cdn.headwindsim.net';
const CDN_PURGE_LINK = 'https://cdn.headwindsim.net/purgeCache?url=https://cdn.headwindsim.net';

const BUCKET_DESTINATION = process.argv[2];
const DIRECTORY = process.argv[3];
const CDN_DIR = BUCKET_DESTINATION ?? 'addons/simlink/test';
const LOCAL_DIR = DIRECTORY ?? './build-modules/';

const PASSWORD = process.env.CLOUDFLARE_WORKER_PASSWORD;
...
const uploadFile = async (url, buffer) => {
    ...
    try {
        const putOptions = {
            method: 'PUT',
            headers: { 'X-HDW-Access-Key': PASSWORD },
            body: buffer,
        };
